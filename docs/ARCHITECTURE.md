...
Ollama на слабой машине отдаёт 3–10 токенов в секунду. Обновление
...
**Завершение стрима определяется флагом `done` от Ollama.** Ollama
стримит NDJSON, в финальном чанке которого стоит `done=True`.
`OllamaService.stream_chat` прокидывает этот флаг наверх,
`StreamManager._run_worker` при его получении выходит из цикла
немедленно — не дожидаясь EOF/close от httpx. Это критично для
UX: ожидание закрытия соединения занимало секунды, и всё это время
composer в UI оставался заблокированным.
...
## Фоновый поток стрима
    ...
    _run_worker(message_id, model, messages):
      ...
      │     └─ yield'ит пары (content, done)
      │
      ├─ for content, done in generator:
      │     with self._lock:
      │         state = self._streams.get(message_id)
      │         if cancelled: break
      │         if content: state.chunks.append(content)
      │         if done: break      ← немедленный выход, не ждём EOF
      │
...
**Почему `done` важен.** До появления флага `done` в обработке
цикл `for chunk in generator` завершался только когда httpx
дочитывал соединение и освобождал ресурсы. На практике это могло
занимать несколько секунд после последнего токена. Всё это время
composer в UI оставался заблокированным. Флаг `done` из финального
чанка Ollama позволяет завершить стрим в тот же момент, когда
модель закончила генерацию, не ожидая никакого I/O.

## Синхронизация composer'а на клиенте

Состояние composer'а (заблокирован / активен) определяется наличием
`.message-polling` в DOM: пока polling-элемент есть — стрим идёт.

Проверка выполняется через `setInterval(updateComposerState, 500)`
в `base.html`. Не через `htmx:afterSwap`: HTMX при `hx-swap="outerHTML"`
эмитит события на уже отсоединённом узле, и они не всплывают до
`document.body`.

Когда `busy` переходит из `true` в `false`, функция возвращает
фокус в textarea и подсвечивает его короткой CSS-анимацией.
Фокус не крадётся, если пользователь сейчас в другой вкладке или
печатает в другом поле.
